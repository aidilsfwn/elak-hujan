...
export function Settings() {
  ...
  const save = () => { if (errors.length) return; setConfig({ ...draft, configVersion: 2 }); setSaved(true); window.setTimeout(() => setSaved(false), 2000); };
  ...
  return <div className="settings-page">
    ...
    <div className="settings-layout">
      <div className="settings-form">
        ...
        <section className="settings-section"><header><span>03</span><div><h2>Rutin pejabat</h2><p>Pilih bilangan hari dan sekurang-kurangnya jumlah hari pilihan yang sama.</p></div></header><div className="routine-setting"><div><span>Hari seminggu</span><div>{[1,2,3,4,5].map((count) => <button type="button" key={count} onClick={() => update({ officeDaysPerWeek: count })} className={draft.officeDaysPerWeek === count ? 'is-selected' : ''}>{count}</button>)}</div></div><div><span>Hari pilihan</span><div>{weekdays.map((day) => { const selected = draft.preferredDays.includes(day.key); return <button type="button" key={day.key} onClick={() => update({ preferredDays: selected ? draft.preferredDays.filter((item) => item !== day.key) : [...draft.preferredDays, day.key] })} className={selected ? 'is-selected' : ''}>{day.label}</button>; })}</div></div></div></section>
