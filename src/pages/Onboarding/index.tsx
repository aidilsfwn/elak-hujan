...
function valid(step: number, draft: Partial<UserConfig>) { if (step === 0) return isValidLocation(draft.homeLocation) && isValidLocation(draft.officeLocation); if (step === 1) return Boolean(draft.morningWindow && draft.eveningWindow && isValidTimeWindow(draft.morningWindow.start, draft.morningWindow.end) && isValidTimeWindow(draft.eveningWindow.start, draft.eveningWindow.end)); return Boolean(draft.officeDaysPerWeek && draft.preferredDays && draft.preferredDays.length >= draft.officeDaysPerWeek); }
...
export function Onboarding() {
  ...
  const next = () => { if (step < 2) return setStep(step + 1); setConfig({ homeLocation: draft.homeLocation!, officeLocation: draft.officeLocation!, morningWindow: draft.morningWindow!, eveningWindow: draft.eveningWindow!, officeDaysPerWeek: draft.officeDaysPerWeek!, preferredDays: draft.preferredDays!, rainThreshold: draft.rainThreshold ?? 40, onboardingComplete: true, configVersion: 2 }); navigate('/leave', { replace: true }); };
